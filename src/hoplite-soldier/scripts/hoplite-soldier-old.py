...
class PotentialFieldCollisionAvoidance(object):
    ...
    def __init__(self, robot_radius, detection_radius, max_velocity=None):
        ...
        self.distance_power = 2.0        # How quickly force decreases with distance (usually 1 or 2)
    ...
    def compute_velocity_adjustment(self, pos, vel, neighbors):
        ...
        for neighbor_pos, neighbor_vel, neighbor_radius in neighbors:
            ...
            # Calculate safety distance (sum of radii plus a safety margin)
            safety_dist = self.robot_radius + neighbor_radius + 50.0  # 50mm safety margin
            ...
            if dist < self.detection_radius:
                ...
                if dist > 0:
                    ...
                    # If we're exactly at the same position (shouldn't happen normally)
                    force_dir_x, force_dir_y = 1.0, 0.0
        ...
        # Scale force effect based on a time step (can be tuned)
        time_factor = 0.05  # Smaller value for smoother adjustments
        adjusted_vel[0] += total_force_x * time_factor
        adjusted_vel[1] += total_force_y * time_factor
...
class HopliteKinematicModel(object):
    ...
    def __init__(self):
        ...
        self.linear_acc = 7500.0
        self.angular_acc = math.pi / 4.0
        self.turn_acc = 4 * math.pi
...
class HopliteSoldierNode(object):
    def __init__(self):
        ...
        self.repulsive_coeff = rospy.get_param("~repulsive_coeff", 500000.0)  # Force strength
